...
class DoctorsTransactionTable extends LivewireTableComponent
{
    ...
    public function builder(): Builder
    {
        ...
        if ($this->dateFilter != '' && $this->dateFilter != getWeekDate()) {
            ...
            $startDate = Carbon::createFromFormat('d/m/Y', $timeEntryDate[0])->startOfDay();
            $endDate = Carbon::createFromFormat('d/m/Y', $timeEntryDate[1])->endOfDay();
            $transaction->whereBetween('transactions.created_at', [$startDate, $endDate]);
        } else {
            $timeEntryDate = explode(' - ', getWeekDate());
            $startDate = Carbon::parse($timeEntryDate[0])->startOfDay();
            $endDate = Carbon::parse($timeEntryDate[1])->endOfDay();
            $transaction->whereBetween('transactions.created_at', [$startDate, $endDate]);
        }
